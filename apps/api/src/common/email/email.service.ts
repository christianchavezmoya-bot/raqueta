...
export class EmailService {
  ...
  async send2FACode(to: string, code: string): Promise<void> {
    const subject = '???? Tu c??digo de verificaci??n ??? N-Go';
    const html = `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
        <h2 style="color:#1b4a86">C??digo de verificaci??n</h2>
  ...
  async sendPasswordResetEmail(to: string, token: string): Promise<void> {
    ...
    const subject = '???? Restablecer contrase??a ??? N-Go';
    const html = `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
        <h2 style="color:#1b4a86">Restablecer contrase??a</h2>
...
${body}`);
  ...
  private async deliver(to: string, subject: string, html: string, text: string): Promise<void> {
    const from = this.config.get<string>('SMTP_FROM', 'noreply@n-go.app');
    if (!this.transporter) {
